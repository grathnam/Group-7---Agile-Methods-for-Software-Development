...
public class TreeUtils {
	...
	public void printbdeexceedcurrentdate() {
		...
		tree.getPeople().stream().filter(p -> p.getBirth().isAfter(LocalDate.now()))
				.forEach(p -> System.out.println(p.getName() + " has an invalid birthday of: " + p.getBirth()));
		printHeader("Death Date exceeding current date");
		tree.getPeople().stream().filter(p -> p.getDeath() != null && p.getDeath().isAfter(LocalDate.now()))
				.forEach(System.out::println);
		tree.getPeople().stream().filter(p -> p.getDeath() != null && p.getDeath().isAfter(LocalDate.now()))
				.forEach(p -> System.out.println(p.getName() + " has an invalid death day of: " + p.getDeath()));
	...
	public void printmardivdateexcurdate() // US 01.2
	{
		...
		tree.getFamilies().stream().filter(f -> f.getDiv() != null && f.getDiv().isAfter(LocalDate.now())).forEach(
				f -> System.out.println(" " + f.getMembers() + " has an invalid divorse date of: " + f.getDiv()));
		printHeader("Marriage Date exceeding current date");
		tree.getFamilies().stream().filter(f -> f.getMarr() != null && f.getMarr().isAfter(LocalDate.now())).forEach(
				f -> System.out.println(" " + f.getMembers() + " has an invalid marriage date of: " + f.getMarr()));
	}
			...
			for (Family f : p.getFams()) {
				if (f.getMarr() != null && p.getDeath() != null) {
					if (f.getMarr().isAfter(p.getDeath())) {
						System.out.println("  " + p.getName() + " has  marriage date: " + f.getMarr() + " and "
			...
			for (Family f : p.getFams()) { // beginning of inner for loop
				if (f.getMarr() != null && p.getBirth() != null) { // beginning
					...
					if (f.getMarr().isBefore(p.getBirth())) { // beginning of
						...
						System.out.println("  " + p.getName() + " has marraige date : " + f.getMarr() + " and "
								+ " Birth date:  " + p.getBirth());
			...
			for (Family f : p.getFams()) { // beginning of inner for loop
				if (f.getDiv() != null && p.getDeath() != null) { // beginning
					...
					if (f.getDiv().isAfter(p.getDeath())) { // beginning of
						...
						System.out.println(" " + p.getName() + " has  divorce date: " + f.getDiv()
...

		{
			if (p.getBirth() != null && p.getDeath() != null) {
				if (p.getBirth().isAfter(p.getDeath())) {
					System.out.println("  " + p.getName() + " has  birth date: " + p.getBirth() + " and "
							+ " Death date:  " + p.getDeath());
		...
		for (Person p : tree.getPeople()) // beginning of for stmt
		{
			if (p.getBirth() != null && p.getDeath() != null) { // outer if stmt
				if ((p.getBirth().isBefore(LocalDate.now()) && (p.getDeath().isBefore(LocalDate.now())))) {
					if (((p.getDeath().isAfter(p.getBirth()))) && (p.getDeath().minusYears(150) != (p.getBirth()))) // checks
																													// if
																													// death
																													// date
																													// is
	...
	public void printchkcurdtlesthan150yrsbddate() {
		...
		for (Person p : tree.getPeople()) // beginning of for stmt
		{
			if (p.getBirth() != null && p.getDeath() == null) { // outer if stmt
				...
				if ((p.getBirth().isBefore(LocalDate.now())) && (LocalDate.now().minusYears(150) != (p.getBirth()))) { // inner
																														// if
																														// stmt
																														// begins
	...
	public void listSiblingsOrdedByAge() {
		...
		tree.getFamilies().stream()
				.collect(Collectors.toMap(f -> f,
						f -> f.getChild().stream().sorted((p1, p2) -> p1.getBirth().compareTo(p2.getBirth()))
								.collect(Collectors.toList())))
				.entrySet().stream().filter(e -> !e.getValue().isEmpty()).forEach(e -> {
	...
	public void printIncorrectGenderRoles() {
		...
		tree.getPeople().stream()
				.filter(p -> !p.getFams().stream()
						.filter(f -> (p.getSex().equalsIgnoreCase("M") && f.getWife().getId().equals(p.getId())) ? true
								: (p.getSex().equalsIgnoreCase("F") && f.getHusband().getId().equals(p.getId())))
	...
	public void listNonUniqueNamesAndBirthdays() {
		...
		tree.getPeople().stream()
				.collect(Collectors.toMap(p -> p.getName().concat(p.getBirth().toString()),
						v -> new ArrayList(Arrays.asList(v)), (v1, v2) -> v1.addAll(v2) ? v1 : v1))
	...
	public void listNonUniqueNamesInFamily() {
		...
		tree.getFamilies().stream()
				.collect(Collectors.toMap(f -> f, f -> f.getMembers().stream()
						.collect(Collectors.toMap(p -> p.getName().split(" ", 2)[0].concat(p.getBirth().toString()),
								v -> new ArrayList(Arrays.asList(v)), (v1, v2) -> v1.addAll(v2) ? v1 : v1))
						.entrySet().stream().filter(e -> e.getValue().size() > 1).map(e -> e.getValue())
						.collect(Collectors.toList())))
				.entrySet().stream().filter(e -> !e.getValue().isEmpty()).forEach(e -> {
					System.out.println(e.getKey());
					e.getValue().forEach(p -> System.out.println(" >\t" + p));
	...
	public void listLivingSingles() {
		...
		tree.getPeople().stream().filter(p -> (p.getFams().size() == 0) && AgeUtils.getAge(p.getBirth()) > 30)
				.forEach(System.out::println);
	}
	...
	public void listMultipleBirths() {
		...
		tree.getPeople().stream()
				.collect(Collectors.toMap(p -> p.getBirth(), v -> new ArrayList(Arrays.asList(v)),
	...
	public void listMarriageNotAfter14() {
		...
		tree.getFamilies().stream()
				.filter(f -> AgeUtils.getTimeGap(0, f.getHusband().getBirth(), f.getMarr()) < 14
						|| AgeUtils.getTimeGap(0, f.getWife().getBirth(), f.getMarr()) < 14)
				.forEach(System.out::println);
	}
	...
	public void listFamilyNotFewerThan15Siblings() {
		...
		tree.getFamilies().stream().filter(f -> f.getChild().size() >= 15).forEach(System.out::println);
	}
	...
	public void listBirthAfterParentsDeath() {
		...
		tree.getFamilies()
				.forEach(f -> f.getChild().stream()
						.filter(c -> (f.getHusband().getDeath() != null
								&& AgeUtils.getTimeGap(1, c.getBirth(), f.getHusband().getDeath()) > 9)
								|| (f.getWife().getDeath() != null
										&& AgeUtils.getTimeGap(1, c.getBirth(), f.getWife().getDeath()) > 0))
				.forEach(System.out::println));
	...
	public void printParentTooOld() {
		...
		tree.getFamilies().stream()
				.forEach(f -> f.getChild().stream()
						.filter(c -> AgeUtils.getTimeGap(0, f.getHusband().getBirth(), c.getBirth()) >= 80
								|| AgeUtils.getTimeGap(0, f.getWife().getBirth(), c.getBirth()) >= 60)
				.forEach(System.out::println));
	...
	public void printMaleLastName() {
		printHeader("Printing male members don't have the right last name");
		tree.getFamilies().stream()
				.forEach(
						f -> f.getChild().stream()
								.filter(c -> c.getSex().equals("M") && !AgeUtils.getLastName(c.getName())
										.equals(AgeUtils.getLastName(f.getHusband().getName())))
				.forEach(System.out::println));
	}
	...
	public void printLivingMarried() {
		...
		tree.getFamilies().stream().filter(f -> f.getDiv() == null)
				.map(f -> new ArrayList<Person>(Arrays.asList(f.getWife(), f.getHusband()))).flatMap(l -> l.stream())
				.filter(p -> p.getDeath() == null && AgeUtils.getAge(p.getBirth()) > 0).forEach(System.out::println);
	...
	public void printSiblingSpacingInvalid() {
		printHeader("Printing Birth dates of siblings should be more than 8 months apart or less than 2 days apart.");
		/*tree.getFamilies().stream().forEach(f -> f.getChild().stream().forEach(e -> {
			new ArrayList(Arrays.asList(e));
		}));*/
		for (Family f : tree.getFamilies()) {
			for (Person p : f.getChild()) {
				for (int i = 0; i < f.getChild().size(); i++) {
					long day = AgeUtils.getTimeGap(2, p.getBirth(), f.getChild().get(i).getBirth());
					if (day > 2) {
						long month = AgeUtils.getTimeGap(1, p.getBirth(), f.getChild().get(i).getBirth());
						if (month < 8) {
